...
wss.on("connection", function connection(ws) {
        ...
        switch (parsedData.type) {
            case "connection":
                ...
                wss.clients.forEach(client => {
                    if (client != ws)
                        client.send(JSON.stringify({type: "newCursor", guid}))
                })
            ...
            case "mouseMove":
                ...
                wss.clients.forEach(client => {
                    if (client != ws)
                        client.send(JSON.stringify({ type, guid: ws.guid, mouseX: parsedData.mouseX, mouseY: parsedData.mouseY }));
                });
                break;
            default:
    ...
    ws.on("close", function close() {
        wss.clients.forEach(client => {
            if (client != ws)
                client.send(JSON.stringify({type: "deleteCursor", guid: ws.guid}))
        })
    })
});
